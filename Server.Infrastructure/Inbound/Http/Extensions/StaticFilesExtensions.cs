...
namespace Server.Infrastructure.Inbound.Http.Extensions;
...
public static class StaticFilesExtensions
{
    ...
    public static WebApplication UseWebInterface(this WebApplication app, IWebHostEnvironment env)
    {
        ...
        var path = Path.Combine(AppContext.BaseDirectory, "Server.Infrastructure", "Inbound", "Http", "wwwroot");
        
        // Se estamos em ambiente de desenvolvimento, tentamos usar o caminho relativo ao projeto
        if (env.IsDevelopment())
        {
            var devPath = Path.GetFullPath(Path.Combine(env.ContentRootPath, "..", "Server.Infrastructure", "Inbound", "Http", "wwwroot"));
